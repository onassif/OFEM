classdef CPmini
   ...
   methods
      %% Construct
      function ob = CPmini(num, props, cpType, cpProps, angles, slip, time, identity)
         ob.ndm   = num.ndm;
         ob.nstr  = num.str;
         ob.ngp   = num.gp;
         ob.list.D = zeros(   6, 6, num.gp, num.el2);
         ...
         ob.list.R    = zeros(3, 3, num.gp, num.el2);
      ...
      function [gp, el, ob] = computeKFM(ob, gp, el, step)
         ...
         if el.iter == 0 && step > 1
            ...
            de    = ob.de;        ob.cpM.de    = ob.de;
            deEff = np1.deEff;    ob.cpM.deEff = deEff;
            ...
            ob.list.D( :,:,gp.i,el.i) = D; % [116770.243255197,57513.7019018134,57513.7019018134,0,0,0[29628.2706766917]]
            ob.list.Rp(:,:,gp.i,el.i, step+1) = expm(wbarp)*ob.list.Rp(:,:,gp.i,el.i, step); %eye(3)
            ob.list.R( :,:,gp.i,el.i) = check2D(gp.R); %eye(3)
