    ...
    function ob = CP(...
      ...
      for i=1:length(angles)
        switch angles{i,1}
          ...
          otherwise
            error( [...
              'You''ve chosen mm10 material but specified incompatible ',...
      ...
      ob.nslip  = slip.nslip;
      ob.slip.n = slip.n;
      [ob.gRot, ob.schmidt] = gRotSchmidt(...
        ob.angles.val, ...
        slip.b, ...
        slip.n, ...
        slip.nslip);
...
%             dt   = ob.dt(step);
        ...
        while (frac<1)
          ...
          while (...
            (norm(R1)>ob.atol1) && (norm(R1)/nrm.iR1>ob.rtol1) ||...
            (norm(R2)>ob.atol2) && (norm(R2)/nrm.iR2>ob.rtol2))
            ...
            iter = iter + 1;
            % Increment and check for failure
